...
func TestConclusion(t *testing.T) {
	cases := []struct {
		...
		{"gated awaiting stays in progress", snapshot{finalized: true, totalGates: 2, activeGates: 1}, ""},
		{"gated denied is action_required", snapshot{finalized: true, totalGates: 2, activeGates: 1, deniedGates: 1}, "action_required"},
...
func TestConclusionLockPrecedence(t *testing.T) {
	...
	cases := []struct {
		...
		{"awaiting gate stays in progress over lock", snapshot{finalized: true, totalGates: 1}, held, ""},
		{"denied gate beats lock", snapshot{finalized: true, totalGates: 1, deniedGates: 1}, held, "action_required"},
