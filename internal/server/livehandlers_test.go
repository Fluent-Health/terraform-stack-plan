...
func TestStackDetailPage(t *testing.T) {
	...
	s := string(body)
	// Plan kind: Log + Plan tabs, no Verify tab.
	for _, want := range []string{
		"stacks/a", "PLAN_SECTION_A", "LOG_TAIL_A",
		`/logs/e1/stacks/a`, "tabs", "Plan", "/assets/app.css",
	} {
		if !strings.Contains(s, want) {
			t.Errorf("stack page missing %q", want)
		}
	}
	if strings.Contains(s, "aria-label=\"Verify\"") {
		t.Error("plan kind stack page must not show Verify tab")
	}
...
func TestStackDetailVerifyLink(t *testing.T) {
	...
	// Apply-kind execution: Verify tab is shown and links to the verify run.
	_ = store.UpsertInit(db, events.Init{ID: "apply-1", Repo: "o/r", PR: 7, Environment: "staging",
		Context: "apply/staging", Stacks: []events.StackState{{Path: "stacks/a"}}})
	_ = store.UpsertInit(db, events.Init{ID: "verify-9", Repo: "o/r", PR: 7, Environment: "staging",
		Context: "verify/staging"})

	resp, _ := http.Get(srv.URL + "/live/apply-1/stack/stacks/a")
	body, _ := io.ReadAll(resp.Body)
	resp.Body.Close()
...
func TestStackDetailVerifyInline(t *testing.T) {
	...
	// Apply-kind execution: Verify tab is shown with inline log output.
	_ = store.UpsertInit(db, events.Init{ID: "apply-1", Repo: "o/r", PR: 7, Environment: "staging",
		Context: "apply/staging", Stacks: []events.StackState{{Path: "stacks/a"}}})
	_ = store.UpsertInit(db, events.Init{ID: "verify-9", Repo: "o/r", PR: 7, Environment: "staging",
		Context: "verify/staging"})
...
func TestStackDetailApplyTabsNoVerifyLink(t *testing.T) {
	db := newServerTestDB(t)
	a := New(db, &MockGitHub{}, Config{})
	srv := httptest.NewServer(a.Routes())
	defer srv.Close()

	// Apply-kind execution without a verify run: Verify tab present but shows "no verify run" message.
	_ = store.UpsertInit(db, events.Init{ID: "apply-2", Repo: "o/r", PR: 8, Environment: "staging",
		Context: "apply/staging", Stacks: []events.StackState{{Path: "stacks/b"}}})

	resp, _ := http.Get(srv.URL + "/live/apply-2/stack/stacks/b")
	body, _ := io.ReadAll(resp.Body)
	resp.Body.Close()
	s := string(body)

	if !strings.Contains(s, "aria-label=\"Verify\"") {
		t.Error("apply kind stack page must show Verify tab")
	}
	if strings.Contains(s, "aria-label=\"Plan\"") {
		t.Error("apply kind stack page must not show Plan tab")
	}
}
